...
def mlpSimpleDiv(X, Y):
    ...
    mlp = MLPClassifier(activation="identity", verbose=True,
                        max_iter=150, hidden_layer_sizes=50, early_stopping=True, 
                        tol=1e-12, validation_fraction=0.3, alpha=1e-8, 
                        learning_rate_init=0.0001, beta_1=0.5, warm_start=True) 
    # Prediction and evaluation
    mlp.fit(X_train_n, Y_train)
    prediction = mlp.predict(X_val_n)
    score = mlp.score(X_val_n, Y_val)
    print "Score:", score
    return mlp, scalar
...
"""
def mlpCrossVal(X, Y):
    X_red = FastICA(n_components=50, whiten=True).fit_transform(X,Y) #65
...
def svmCrossVal(X, Y):
    X_red = FastICA(n_components=30, whiten=True).fit_transform(X, Y)
...
def predictTestData(X_train, Y_train, classifier, scalar):
    #X_red_train = FastICA(n_components=50, whiten=True).fit_transform(X_train)
    Xn = scalar.fit_transform(X_train)
    classifier.fit(Xn, Y_train)
        
    test_data = pd.read_csv('testdata.csv')
    X_test, Y_test = analyze_select_data(test_data, False)
   # X_red_test = FastICA(n_components=50, whiten=True).fit_transform(X_test)
    Xn_test = scalar.fit_transform(X_test)
    pred = pd.DataFrame(classifier.predict(Xn_test), index=np.arange(1,201))
